...
void handFollow(){
  ...
  if (leftSensor == LOW && rightSensor == HIGH) { //active low logic, when it detects the presence of hand, the digital output given by IR isLOW
    leftturn();
  }
...
void collisonAvoidance(){
  servo.write(90); //Align the motor such that ultrasonic sensor mounted on it faces frontside
  delay(servo_delay);
  middis=distance();
  Serial.println(middis);
  if(middis <=10){ //when obstacle is close to bot
    stop();
    delay(servo_delay);
    servo.write(30);
    delay(servo_delay);
